...
# test that the same Krylov subspaces are being constructed
b = rand(length(x0))
...
x, ndE = nkminim(E, dE, x0; verbose=3, linesearch = Isaac.lsdefault)
println("   num_dE = ", ndE)
println("   |∇E| = ", norm(dE(x), Inf))

println("Testing nsolistab, 2D LJ Vacancy, R = $(round(Int,R)), precon = $(!(P==I))")
x, ndE = nsolistab(dE, x0; verbose=3, maxnumdE = 1000)
println("   num_dE = ", ndE)
println("   |∇E| = ", norm(dE(x), Inf))
